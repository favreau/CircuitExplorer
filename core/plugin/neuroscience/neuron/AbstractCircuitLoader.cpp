    ...
    const brain::Circuit &circuit, GIDOffsets &targetGIDOffsets) const
{
    ...
    if (circuitTargets.empty())
    {
        ...
        PLUGIN_INFO(
            "- No target specified. Loading default one: " << defaultTarget);
    }
    else
...
CompartmentReportPtr AbstractCircuitLoader::_attachSimulationHandler(
    const PropertyMap &properties, const brion::BlueConfig &blueConfiguration,
    Model &model, const ReportType &reportType, brain::GIDSet &gids) const
{
...
ModelDescriptorPtr AbstractCircuitLoader::importCircuit(
    const std::string &circuitConfiguration, const PropertyMap &properties,
    const LoaderProgress &callback) const
{
        ...
        if (compartmentReport != nullptr)
            ...
            maxMorphologyLength =
                _importMorphologies(properties, circuit, *model, allGids,
                                    ...
                                    callback, SECONDARY_MODEL_MATERIAL_ID);
    }
    ...
    ModelMetadata metadata = {
        ...
        {"RandomSeed",
         std::to_string(properties.getProperty<double>(PROP_RANDOM_SEED.name))},
    ...
    modelDescriptor =
        std::make_shared<ModelDescriptor>(std::move(model), "Circuit",
                                          circuitConfiguration, metadata);
    modelDescriptor->setTransformation(transformation);
    return modelDescriptor;
}

size_t AbstractCircuitLoader::_getMaterialFromCircuitAttributes(
    const PropertyMap &properties, const uint64_t index, const size_t material,
...
size_ts AbstractCircuitLoader::_populateLayerIds(
    const PropertyMap &properties, const brion::BlueConfig &blueConfig,
...
std::string AbstractCircuitLoader::_getMeshFilenameFromGID(
    const PropertyMap &properties, const uint64_t gid) const
{
    const auto meshFolder =
        properties.getProperty<std::string>(PROP_MESH_FOLDER.name);
    const auto meshFilenamePattern =
...
void AbstractCircuitLoader::_importMeshes(
    const PropertyMap &properties, Model &model, const brain::GIDSet &gids,
    ...
    const size_ts &electrophysiologyTypes, const LoaderProgress &callback) const
{
    ...
    for (const auto &gid : gids)
    {
        const size_t materialId =
            _getMaterialFromCircuitAttributes(properties, meshIndex,
                                              NO_MATERIAL, targetGIDOffsets,
                                              layerIds, morphologyTypes,
                                              electrophysiologyTypes, false);

        // Load mesh from file
        const auto transformation =
            meshTransformation ? transformations[meshIndex] : Matrix4f();
        ...
        try
        {
            meshLoader.importMesh(_getMeshFilenameFromGID(properties, gid),
                                  LoaderProgress(), model, transformation,
                                  materialId, quality);
        }
...
#else
void AbstractCircuitLoader::_importMeshes(
    const PropertyMap & /*props*/, Model & /*model*/,
...
void AbstractCircuitLoader::setSimulationTransferFunction(
    TransferFunction &tf, const float finalOpacity)
{
...
float AbstractCircuitLoader::_importMorphologies(
    const PropertyMap &properties, const brain::Circuit &circuit, Model &model,
    const brain::GIDSet &gids, const Matrix4fs &transformations,
...
#pragma omp critical
    ...
    for (size_t i = 0; i < containers.size(); ++i)
    {
        ...
        maxDistanceToSoma =
            std::max(container.getMorphologyInfo().maxDistanceToSoma,
                     maxDistanceToSoma);
